...
\def\refProjet{9} 
\def\titreProjetCourt{Performances MPTCP}
\def\titreProjetLong{Performances MPTCP sur différentes topologies virtuelles}
...
Ici doit prendre place votre cahier des charges. Le présent document est
un modèle de mise en forme des rapports que vous devez fournir pour cette UE.
Dans la suite de cette section nous allons donc plutôt développer les points
suivants:

\subsection{Contenu des rapports}
Deux rapports sont à fournir pour l'évaluation d'un projet: l'intermédiaire et
le final.
...
\begin{itemize}
\item d'un cahier des charges (200 à 400 mots),
\item d'un plan de développement (200 à 300 mots plus un
  diagramme de Gantt pour visualiser le séquencement des tâches réalisées par
  \textbf{chaque} étudiant),
\item d'une présentation du contexte technologique (600 à 2000 mots),
\item d'une analyse (800 à 1600 mots),
\item d'une conception (800 à 1600 mots),
\item d'un état d'avancement du projet (200 à 300 mots justifiant l'état de
...
\item de la présentation du contexte technologique \textbf{actualisée} (600 à 2000 mots),
\item de l'analyse \textbf{actualisée} (800 à 1600 mots),
\item de la conception \textbf{actualisée} (800 à 1600 mots),
\item d'un \textbf{compte rendu du projet} (2000 à 3000 mots, précisant le déroulement,
...
modèles \LaTeX, OpenDocument et MSWord seront disponibles.
...
La police de caractère utilisée devra être une police avec sérif de type
...
L'en-tête indique à droite l'université en \textsc{petites capitales}, le
Master Informatique, le nom de l'UE avec l'année universitaire en cours puis
le nom de l'encadrant (initiale du prénom puis nom de famille avec seule la
première lettre en majuscule). Au centre de l'en-tête est indiqué en
\textbf{gras} la référence du projet (son numéro) et son titre résumé (une
vingtaine de caractères). A gauche de l'en-tête doit se trouver le nom des
étudiants (initiale du prénom puis nom de famille avec seule la première
...
Le pied de page comporte à droite le nouveau logo de l'UPMC sur 3 cm de large, le numéro de la page relatif par rapport au nombre total de pages en \textbf{gras} au centre et à gauche le type de rapport (intermédaire ou final).

\subsection{Transmission des rapports}
Avant tout envoi d'un rapport définitif, une version préliminaire est à envoyer à votre encadrant afin qu'il avalise votre travail. Les rapports définitifs sont à télécharger en format \textbf{PDF}\footnote{Pour vérifier si votre PDF est portable, envoyez-le en test sur une machine sous GNU/Linux et lisez-le avec le reader pdf local.} aux dates spécifiées et via les pages web indiquées. 
