...
(mesuré avec la commande \emph{ping}), ce qui correspond au délai
attendu pour traverser deux liens. La moyenne ici tient compte du RTT
du premier paquet qui est envoyé vers le contrôleur pour que celui-ci
établisse le chemin vers le serveur.
...
interfaces. Nous observons que pour deux sous-flots aux capacités
identiques et au coût identique, le débit mesuré est quasi similaire
(une différence de quelques paquets est observée).
...
Ce problème pourrait être expliquer par une utilisation non optimale
...
à 100 Mbit/s. C'est pourquoi, nous limiterons la capacité des liens à
de faibles valeurs.
...
faisons varier le délai de tous les liens de manière symétrique. Le
...
délai par TCP car cette méthode est plus fiable et l'utilisation
...
diminution des performances pour les liens à faibles capacits
...
primordial dans pour les performances de MPTCP. Dans une utilisation
mobile, les deux sous-flots ont généralement des délais différents si
on se base sur l'utilisation conjointe d'un accès wifi, 3G ou/et
ethernet. Dans la figure \ref{fig:mptcp:rttetTC}, nous utilisons les
mêmes paramètres que dans l'expérience précédente cependant le premier
sous-flot a un RTT fixe égal à une dizaine de millisecondes
(expériences ``TC''). Nous observons aucune différence entre le cas où
les deux sous-flots ont un RTT variable et le cas où un seul sous-flot
possède un RTT sauf dans l'expérience ``200 TC'' où le débit est plus
élevé. Nous concluons que la diminution de performance observée pour
la capacité à 200\,Mbit/s est lié à la taille du tampon.
