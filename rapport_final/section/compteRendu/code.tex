...
Une partie de ce projet consiste dans un premier temps en l'étude du
code de MPTCP et de la compréhension de son fonctionnement, puis de
...
\subsection{Etude de  l'algorithme de MPTCP}
 	...
 	Lors de l'étude du code de MPTCP, nous avons étudié
        l'ordonnanceur déjà implémenté. L'ordonnanceur de MPTCP tient
        dans la fonction get\_available\_subflow(), qui se trouve dans
        (\$SRC NOYAU)/net/mptcp/mptcp.output.c. Cela nous a été
        confirmé par Matthieu Coudron lorsqu'il a choisi
        l'ordonnanceur à implémenter. Cette fonction retourne la
        socket sur laquelle sera envoyée le prochain segment de
        données et définit la taille du Maximum Segment Size
 	...
 	Tout d'abord la structure mptcp\_cb signifie \emph{MPTCP
        ...
        utilisée dans pratiquement toutes les fonctions de
        MPTCP. Cette structure permet de superviser les différents
        sous-flots utilisés pour une connexion MPTCP. Elle permet
        aussi de décider s'il faut ouvrir ou fermer un sous-flot, elle
        réordonne les données reçues afin que l'application qui a
        besoin de ces données les obtienne dans le bon ordre.
        \\
 	La structure sock (socket) et la structure sk\_buff (socket
        ...
        et permet de faire un tri sur les données reçues grâce aux
        timestamps.  L'ordonnanceur implémenté dans MPTCP utilise,
        comme dans notre implémentation, le Smoothed Round Trip Time
 	 ...
 	 \begin{itemize}
         ...
         \item Si il y a d'autres sockets qui n'ont pas une faible priorité, on compare le SRTT de ces sockets et on garde la socket avec le RTT le plus faible. Elle est stockée dans la variable \emph{bestsk}.\\
           Il y a aussi une backup socket qui sert au cas où aucune
           des sockets décrites ci-dessus ne peuvent envoyer un
           segment de données : c'est le dernier recours car celle-ci
           est désignée alors qu'elle ne permet pas la réinjection de
           données. Elle sera stockée dans la variable
           \emph{backupsk}.
...
\end{itemize} 
 	...
 	\subsection{Choix de l'ordonnanceur}
        ...
        implémentation.  \bigbreak Dans un premier temps, nous avons
        pensé implémenter un algorithme assez simpliste mais qui
        permettait d'augmenter la sécurité contre les attaques de type
        \emph{Man In The Middle}. C'est à dire que notre algorithme
        allait envoyer les segments de manière équitable sur chaque
        sous-flot permettant l'envoi de données. Cet algorithme avait
        pour vocation de rendre plus difficile la récupération
        d'informations en écoutant un sous-flot car avec l'algorithme
        actuel, si un attaquant voulait récupérer un maximum
        d'informations, il lui suffisait d'écouter le sous-flot qui a
        le SRTT le plus faible. C'est ce que nous voulions éviter avec
        notre algorithme. Cependant après de plus amples réflexions,
        nous avons remarqué que notre algorithme avait un grand nombre
        de défauts.
        \\
        En effet, MPTCP a été développé afin d'améliorer les
        débits. Cet algorithme n'aurai pas eu l'effet escompté si le
        débit d'un sous-flot était vraiment faible comparé aux autres
        sous-flots alors, ce qui influencerait sur les performances de
        MPTCP. C'est pourquoi cet algorithme n'a pas été choisi.
        \bigbreak Un compromis entre sécurité et performance est
...
\end{itemize}
Après en avoir discuté entre nous, nous avons décidé d'utiliser une
liste chainée car les listes chainées permettent une plus grande
flexibilité de manipulation. Nous avons donc déclaré dans
...
On aura une \emph{selected\_sk} qui sera appelée \emph{bssk}. C'est la
"best selected socket". Ce sera la socket sélectionnée qui aura le
meilleur RTT, puis son attribut next pointera sur la deuxième socket
...
\subsection{Debug de l'ordonnanceur implémenté}
On a implémenté un ordonnanceur mais il faut le tester dans les
topologies virtualisées. Divers problèmes ont été rencontrés avec la
machine virtuelle mininet lorsque l'on a voulu utiliser MPTCP. En
effet, à cause de l'hétérogénéité de nos machines physiques, nous
devions chacun faire nos propres corrections pour compiler le noyau
(problèmes de drivers à la compilation, de machine virtuelle, de
package openvswitch). Mais après les avoir corrigés, nous avons une
implémentation pas encore fonctionnelle. En effet, lors de l'exécution
...
à l'aide de User Mode Linux (UML). Nous espérons qu'elle sera
opérationnelle d'ici la soutenance et que l'on pourra mener quelques
tests de performances sur notre implémentation.
