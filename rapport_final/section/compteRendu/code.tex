...
On a implémenté un ordonnanceur mais il faut le tester dans les topologies virtualisées. Divers problèmes ont été rencontrés avec la machine virtuelle mininet lorsque l'on a voulu utiliser MPTCP. En effet, à cause de l'hétérogénéité de nos machines physiques, nous devions chacun faire nos propres corrections pour compiler le noyau (problèmes de drivers à la compilation, de machine virtuelle, de package openvswitch). Mais après les avoir corrigés, nous avons une implémentation pas encore fonctionnelle. En effet, lors de l'exécution du script python de test de MPTCP, on obtient un "kernel panic" sur la fonction d'ordonnancement. Cette fonction est toujours en correction, à l'aide de User Mode Linux (UML). Nous espérons qu'elle sera opérationnel d'ici la soutenance et que l'on pourra mener quelques tests de performances sur notre implémentation.